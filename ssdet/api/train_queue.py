...
from ssdet.cfg import EVENT_STREAM, TASK_HISTORY, TRAIN_LOG, taskSeqDir, trainLogDir
from ssdet.core.events import EventBus, JsonlEventSink
from ssdet.core.tasking import TaskArchiveRepository, TaskConfigMaterializer, TaskProjectionResolver
...
class TrainProcess:
    ...
    def run(self, configfile: str) -> None:
        ...
        train_script = str(repo_root() / "ssdet" / "scripts" / "train.py")
    ...
    def _deploy_after_train(self, configs: dict) -> None:
        ...
        deploy_script = repo_root() / "ssdet" / "scripts" / "deploy.py"
        cmd = [
            sys.executable,
            str(deploy_script),
            "--save-dir",
