...
from ssdet.core.tasking import TaskArchiveRepository, TaskTopologyCompiler
from ssdet.inference import ssDet
from ssdet.utils.tool import get_history_epoch
...
class DatasetHandler:
    def build(self, payload: Dict[str, str]) -> Dict[str, str]:
...
class TrainStatus:
    def read_task_status(self) -> dict:
    ...
    def read_model_log(self, run: ModelRun) -> List[List[str]]:
        log_file = os.path.join(run.model_dir, "train_log.txt")
        with open(log_file, "r") as file:
            return [line.strip().split(",") for line in file.readlines() if line.strip()]

    def read_history_epoch(self, run: ModelRun) -> int:
        return get_history_epoch(run.model_dir + "/")

    def read_task_history(self) -> List[str]:
        with open(TASK_HISTORY, "r") as file:
            return file.readlines()

    def read_telemetry_stream(self, run: ModelRun) -> List[Dict]:
        telemetry_file = os.path.join(run.model_dir, "telemetry.jsonl")
        return self._read_jsonl(telemetry_file)

    def read_global_event_stream(self) -> List[Dict]:
        return self._read_jsonl(EVENT_STREAM)

    def build_visual_log(self, run: ModelRun) -> bytes:
        log_file = os.path.join(run.model_dir, "train_log.txt")
        with open(log_file, "r") as file:
    ...
    def _read_jsonl(self, target_path: str) -> List[Dict]:
...
class PredictHandler:
    ...
    def predict(self, encoded_image: str, model_name: str, runs_dir: str) -> Dict:
        run = ModelRun(model_name=model_name, runs_dir=runs_dir or DEFAULT_RUNS_DIR)
        self._save_image(encoded_image)
        self._ensure_runtime_artifact(run)
        srcimg = cv2.imread(self.temp_image_path)
        model = ssDet(conf=0.5, nms=0.5, weight=run.runtime_weight_path)
    ...
    def _save_image(self, encoded_image: str) -> None:
        with open(self.temp_image_path, "wb") as file:
            file.write(base64.b64decode(encoded_image))

    def _ensure_runtime_artifact(self, run: ModelRun) -> None:
        if not os.path.exists(run.best_weight_path):
            raise FileNotFoundError("No model file founded")
        if os.path.exists(run.runtime_weight_path):
            return
        convert_and_save_model(
            argparse.Namespace(
                model=run.model_name,
                weight=run.best_weight_path,
                save_path=run.runtime_weight_path,
