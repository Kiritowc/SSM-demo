"""SSDet module path helpers — repo-relative paths derived from ssdet/configs/default.yaml."""
...
def backbone_bin(*parts: str) -> str:
    return str(Path("ssdet/artifacts/backbones").joinpath(*parts))


def artifact_rel(*parts: str) -> str:
    return str(Path("ssdet/artifacts").joinpath(*parts))


def data_rel(*parts: str) -> str:
    return str(Path("ssdet/data").joinpath(*parts))
...
@lru_cache
def load_ssdet_default() -> dict:
    return load_yaml(get_paths().configs_ssdet / "default.yaml")


def deploy_config() -> dict[str, str]:
    deploy = load_ssdet_default()["deploy"]
    return {
