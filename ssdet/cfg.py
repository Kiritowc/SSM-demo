"""SSDet runtime configuration — lazy-initialized from ssdet/configs/default.yaml."""
...
def _build_task_config(cfg: dict, paths, root: Path) -> dict:
    runs_rel = cfg.get("runs_dir", "ssdet/artifacts/runs")
    return {
...
def ensure_ssdet_runtime() -> None:
    ...
    root = repo_root()
    ssdet_default = load_yaml(paths.configs_ssdet / "default.yaml")
    task_config = _build_task_config(ssdet_default, paths, root)

    runtime_dir = paths.artifacts_ssdet / "runtime"
    _bootstrap_runtime_config(task_config["cfg_yaml"], task_config["obj_labels"], runtime_dir)

    runs = paths.artifacts_ssdet / "runs"
