...
REPO_ROOT = os.environ.get("SSM_ROOT", os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))
UI_ROOT = os.path.join(REPO_ROOT, "apps", "web")
...
class CvModelState:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self._specs = {
            ...
            "ssg_a_coco": {
                "label": "coco",
                "engine": args.coco_engine,
                "names": args.coco_names,
            },
            "ssg_a_pcb": {
                "label": "pcb",
                "engine": args.pcb_engine,
                "names": args.pcb_names,
            },
            "ssg_a_robot_toy": {
                "label": "robot_toy",
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--weight", default="runs/ssg_a_office/run.bin")
    ap.add_argument("--names", default="cv/configs/office.names",
                    help="类名文件，需与权重训练时的类别一致")
    ap.add_argument("--backend", choices=("trt",), default="trt")
    ap.add_argument("--engine", default="artifacts/cv/engines/ssg_a_coco_fp16.engine")
    ap.add_argument(
        "--default-cv-model",
        choices=("none", "ssg_a_robot_toy"),
        default="none",
    )
    ap.add_argument("--coco-engine", default="artifacts/cv/engines/ssg_a_coco_fp16.engine")
    ap.add_argument("--coco-names", default="cv/configs/coco.names")
    ap.add_argument("--pcb-engine", default="artifacts/cv/engines/ssg_a_pcb_fp16.engine")
    ap.add_argument("--pcb-names", default="data/cv/robot_toy/classes.txt")
    ap.add_argument("--robot-toy-engine", default="artifacts/cv/engines/robot_far.engine")
    ap.add_argument("--robot-toy-names", default="data/cv/robot_toy/classes.txt")
    ap.add_argument("--vlm-base-url", default="http://127.0.0.1:8080")
...
def main() -> None:
    ...
    print(
        "Windows: ssh -N -L 18080:127.0.0.1:%d sunshink@<ORIN_IP>" % args.port,
        flush=True,
    )
